...
#code a categorical variable for ramadan/not ramadan

data$scode <- mapvalues(data$in_ram, from = c("0", "1"), to = c("Not Ramadan", "Ramadan"))
...
geo_lev_ram <-  data %>%
        ...
        summarise(sum = median(sum, na.rm = TRUE)) %>%
        ungroup() %>% 
        mutate(geo = factor(geo, levels = geo_lev$geo)) 
     

#lolly plot, total sum of porn viewing Ramadan vs not ramadan cumulative sums over 4 years    
#maybe I should divide values of sum by total # of weeks before displaying???
ggplot(geo_lev_ram, aes(sum, geo)) +   #include label = year if wish to add label to aes
        geom_line(aes(group = geo)) +
        geom_point(aes(color = scode))
         #geom_text(aes(color = scode), check_overlap = TRUE, size = 4, angle = 45)
...
big_diff <- geo_lev_ram %>% 
        spread(scode, sum) %>% 
        group_by(geo) %>% 
        mutate(Max = max(Ramadan, `Not Ramadan`),
               Min = min(Ramadan, `Not Ramadan`),
               Diff = Max / Min - 1) %>% 
           arrange(desc(Diff)) %>%
             filter(Diff > .2)
...
#create labal frame
right_label <- geo_lev_ram %>%
        group_by(geo) %>%
        arrange(desc(sum)) %>%
        top_n(1)

left_label <- geo_lev_ram %>%
        group_by(geo) %>%
        arrange(desc(sum)) %>%
        slice(2)
...
#create ggplot which highlights the countries with more than a 20% change over ramadan
ggplot(geo_lev_ram, aes(sum, geo)) +
        ...
        geom_text(data = right_label, aes(color = scode, label = round(sum, 0)),
                  size = 3, hjust = -.5) +
        geom_text(data = left_label, aes(color = scode, label = round(sum, 0)),
                  size = 3, hjust = 1.5) +
        scale_x_continuous()
...
plot_label <- big_diff %>%
        select(geo, sum = Max, Diff) %>%
        right_join(right_label)

#add signs indicating percent decrease 
...
#add labels to plot and also a nice theme

p + #scale_color_discrete(labels = c("Not Ramadan", "Ramadan")) +
